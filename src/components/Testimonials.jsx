...
<h2 className='text-gray-100 text-xl font-semibold mt-3'>{testimonail.name}</h2>
<p className='text-gray-300 mb-4'>{testimonail.role}</p>
<div className='flex mb-4'>
{Array(testimonail.stars).fill().map((item,
index)=>(
    // 
<img key={index} src={assets.rating} width={20} alt="" />
))}
</div>
<p className='text-center text-sm
text-gray-200'>{testimonail.text}</p>
</div>
</div>
))}
</div>
